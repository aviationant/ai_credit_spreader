...
def find_trades_long(df_contracts, df_trades):
    for index, row in df_contracts.iterrows():
        try:
            ...
            k = 1
            while(index - k >= 0 and
                row.expiry_date == df_contracts.iloc[index-k]["expiry_date"] and
                row.call_put == df_contracts.iloc[index-k]["call_put"] and
                row.call_put == "P"):

                strike_debit = float(df_contracts.iloc[index-k]["strike"])/1000
                bid_debit = float(df_contracts.iloc[index-k]['bid'])
                ask_debit = float(df_contracts.iloc[index-k]['ask'])
                ...
                min_price = abs(float(row.delta)) * (strike_credit - strike_debit)
...
def find_trades_short(df_contracts, df_trades):
    for index, row in df_contracts.iterrows():
        try:
            if row.call_put != "C":
                continue
            ...
            k = 1
            while(index + k < len(df_contracts) and
                row.expiry_date == df_contracts.iloc[index+k]["expiry_date"] and
                row.call_put == df_contracts.iloc[index+k]["call_put"] and
                row.call_put == "C"):
...
def find_trades(df_contracts, df_trades):
    df_trades = find_trades_long(df_contracts, df_trades)
    df_trades = find_trades_short(df_contracts, df_trades)
    df_trades = trade_filter(df_trades)
    df_trades = df_trades.sort_values(by=["Score"], ascending=False).reset_index(drop=True)
